...
package org.apache.spark.sql.mdjdbc
...
import org.apache.spark.sql.execution.datasources.mdjdbc.{DriverRegistry, JDBCOptions, JdbcUtils}
import org.apache.spark.sql.execution.datasources.mdjdbc.connection.ConnectionProvider
...
abstract class JdbcDialect extends Serializable with Logging{
  ...
  def createConnectionFactory(options: JDBCOptions): Int => Connection = {
    ...
    (partitionId: Int) => {
      DriverRegistry.register(options)
      val driver: Driver = DriverRegistry.get(options)
      val connection =
   ...
   */
  @Since("2.3.0")
  protected[mdjdbc] def escapeSql(value: String): String =
  ...
  private[mdjdbc] class JDBCSQLBuilder extends V2ExpressionSQLBuilder {
